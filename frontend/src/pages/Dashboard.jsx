...
function Dashboard() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const [profileRes, historyRes, sessionsRes] = await Promise.all([
        axios.get("https://auth-system-ejul.vercel.app/api/auth/profile", { headers }),
        axios.get("https://auth-system-ejul.vercel.app/api/auth/login-history", { headers }),
        axios.get("https://auth-system-ejul.vercel.app/api/auth/sessions", { headers }),
      ]);
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post(
        "https://auth-system-ejul.vercel.app/api/auth/logout",
        { refreshToken },
        { headers }
      );
  ...
  const handleLogoutAllDevices = async () => {
    try {
      const headers = { Authorization: `Bearer ${accessToken}` };
      await axios.post("https://auth-system-ejul.vercel.app/api/auth/logout-all-devices", {}, { headers });
  ...
  const handleRemoveSession = async (sessionId) => {
    try {
      const headers = { Authorization: `Bearer ${accessToken}` };
      await axios.post(
        "https://auth-system-ejul.vercel.app/api/auth/remove-session",
        { sessionId },
        { headers }
      );
      fetchUserData();
